...
export function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    onColumnFiltersChange:
      handleFiltersChange as OnChangeFn<ColumnFiltersState>,
    ...
    getRowId: (row: TData) => (row as { id?: string | number }).id?.toString() || Math.random().toString(),
  });
  ...
  React.useEffect(() => {
    if (onSelectionChange) {
      const selectedRows = Object.keys(rowSelection)
        .map((key) => {
          const row = data.find(
            (item: TData) => (item as { id?: string | number }).id?.toString() === key || ""
          );
          return row as TData;
        })
        .filter(Boolean);
